...
def _run_prompt_submit(
    ...
    def run():
        ...
        # Durable crash marker: written before the turn runs, cleared in the
        # finally below. Any concluded turn — success, handled error,
        # interrupt — clears it, so a marker that survives means the process
        # died mid-turn; session.resume auto-continues from it. Compression
        # can rotate session_key mid-turn, so remember the key we wrote under.
        ...
        try:
            ...
            with session["history_lock"]:
                ...
                if not turn_error_retained:
                    _clear_inflight_turn(session)
            # The turn concluded (whatever the outcome) — it is no longer
            # crash-interrupted. Clear under both keys in case compression
            # rotated session_key mid-turn.
            clear_turn_marker(marker_home, marker_key)
            current_key = str(session.get("session_key") or "")
            if current_key and current_key != marker_key:
                clear_turn_marker(marker_home, current_key)
